...
const BOT_TOKEN = "8558420243:AAHJFwjm2_wOpCkQeHu-dMO_59NIpMuzNHg";
const RAILWAY_URL = "https://red-panda-production-8a37.up.railway.app";
const WEBAPP_URL = RAILWAY_URL;
...
// ============================================================
//  SQLite DATABASE SETUP (data সার্ভার restart-এও টিকে থাকবে)
// ============================================================
const db = new Database("/app/data/game.db"); // ✅ Fixed path
...
function saveUser(user) {
  db.prepare("INSERT OR REPLACE INTO users (id, data) VALUES (?, ?)")
...
app.post(`/webhook/${BOT_TOKEN}`, async (req, res) => {
  ...
  if (text.startsWith("/start")) {
    await telegramAPI("sendMessage", {
      chat_id: chatId,
      text: "🐾 Red Panda-তে স্বাগতম! নিচের বাটনে ক্লিক করুন এবং খেলা শুরু করুন!",
      reply_markup: {
        inline_keyboard: [[
          {
            text: "🎮 Play Red Panda",
...
async function setWebhook() {
  const result = await telegramAPI("setWebhook", {
    url: `${RAILWAY_URL}/webhook/${BOT_TOKEN}`,
...
function getOrCreateUser(userId, ref) {
  ...
  // Referral bonus
  if (ref && ref !== userId) {
    const refUser = loadUser(ref);
    if (refUser) {
      refUser.balance += 5000;
      refUser.lifetimeEarned += 5000;
...
app.listen(PORT, () => {
  console.log(`✅ Red Panda backend running on port ${PORT}`);
  setWebhook();
});
